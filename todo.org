...
- [ ] Make game-loop take a state atom to copy state into every trip through the game loop
- [ ] Read from *state*
- [ ] Auto generate form of component state selected
...
- Abstraction for adding functionality at the game loop level?
  - This could be useful for tooling
  - Logging

* TODO Remove text and styles from mk-text-sprite-state
