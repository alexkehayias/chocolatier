...
* TODO Make component state mutable
Same benefit if you use transient
Also don't return a new object, return the same one so we are not creating new data structures every time
